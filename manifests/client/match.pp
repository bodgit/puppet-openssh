# Declare a client match block
#
# @example Declaring a match block
...
# @param users
# @param localuser
# @param canonical_negate
# @param exec_negate
# @param host_negate
# @param originalhost_negate
# @param user_negate
# @param localuser_negate
# @param order
...
# @see puppet_defined_types::openssh::client::host ::openssh::client::host
define openssh::client::match (
  Stdlib::Absolutepath                                                                      $target,
  Boolean                                                                                   $canonical                            = false,
  Optional[Array[String, 1]]                                                                $exec                                 = undef,
  Optional[Array[String, 1]]                                                                $host                                 = undef,
  Optional[Array[String, 1]]                                                                $originalhost                         = undef,
  Optional[Array[String, 1]]                                                                $users                                = undef, # Annoying clash
  Optional[Array[String, 1]]                                                                $localuser                            = undef,
  Boolean                                                                                   $canonical_negate                     = false,
  Boolean                                                                                   $exec_negate                          = false,
  Boolean                                                                                   $host_negate                          = false,
  Boolean                                                                                   $originalhost_negate                  = false,
  Boolean                                                                                   $user_negate                          = false,
  Boolean                                                                                   $localuser_negate                     = false,
  ...
  $content = template("${module_name}/client/match.erb")
  ...
  if split($content, '\n')[-1] !~ /^Match / {
    ::concat::fragment { "${module_name} client ${target} match ${title}":
      target  => $target,
      order   => "20-${order}",
      content => $content,
    }
