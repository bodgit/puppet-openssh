# Declare an SSH client configuration file
#
# @example Declaring a user configuration file
...
define openssh::client::configuration (
  Variant[Integer[0], String]                                                               $owner,
  Variant[Integer[0], String]                                                               $group,
  Pattern[/(?x) ^ [0-7]{4} $/]                                                              $mode,
  ...
  Optional[Boolean]                                                                         $enable_ssh_keysign                   = undef,
  ...
  $content = template("${module_name}/client/config.erb")

  # Only include a fragment if there some actual settings applied. If there's
  # nothing other than a single line, there are none
  if length(split($content, '\n')) > 1 {
    ::concat::fragment { "${module_name} client ${file} config":
      target  => $file,
      order   => 10,
      content => template("${module_name}/client/config.erb"),
    }
